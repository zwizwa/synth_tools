...
start_link(#{client := _Client, notify := _Notify}=Config) ->
    Pid = 
        serv:start(
          {handler, 
           fun() ->
...
%% FIXME: Function is not used.  Probably incomplete.

wait(_Client,Ports) ->
    %% FIXME: Register
    _Ref = erlang:make_ref(),
    receive {_Tag, {client,true,Name}} -> ok end,
    [begin 
         PortName = tools:format("~s:~s",[Name,Port]),
         receive {_Tag1, {port,true,PortName}} -> ok end
     end || Port <- Ports],
    %% FIXME: Flush
    ok.
